...
//
// 预测 A：三行 "worker x done" 的顺序固定吗？不一定会按顺序，因为是并发
// 预测 B：把 wg.Wait() 注释掉再跑，会打出几行？一行，all workers done
// 预测 C：worker 参数改成 wg sync.WaitGroup（不带 *）会怎样？（先想再试，试完改回来），好像也可以
func worker(id int, wg *sync.WaitGroup) {
	defer wg.Done()
	// time.Sleep(time.Duration(rand.Intn(100)) * time.Millisecond)
	fmt.Printf("worker x< %d > done\n", id)
}
